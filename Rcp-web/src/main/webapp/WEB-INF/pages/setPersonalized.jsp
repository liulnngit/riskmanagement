									...
									<div>
										<div class="input-group">
											...
											<input type="text" class="form-control" id="merchantId" name="merchantId"
												value="${merchantId}" readonly="readonly" style="width: 40%;">
											</select> 
											<span class="input-group-addon">规则类型:</span> <select
												id="ruleType" name="ruleType" class="form-control"
												style="width: 40%;">
											</select> <span class="input-group-addon" style="margin-left: 10%;">支付类型:</span>
...
<script type="text/javascript">
	$(function() {
		...
		$("#resetBtn").click(function() {
			...
			var url = '';
			if ('default' == payclassCode) {
				url = '${pageContext.request.contextPath}/queryMerchantRules';
			...
			getBusinessData(ruleType, merchantId, payclassId, url);
			checkAll();
			tabSwitch();
		});
...
<script type="text/javascript">
	...
	function getBusinessData(ruleType, merchantId, payclassId, url) {
		var levelId = [];
		var levelName = [];
		$.ajax({
		...
		$
				.ajax({
					...
					url : url,
					type : 'post',
					data : {"ruleType":ruleType,"merchantId":merchantId,"payclassId":payclassId},
					dataType : "json",
					async : false,
					error : function(request) {
						layer.alert('网络异常！', {
								...
								if (data[i].isConfig == '1') {
			                      ...
			                      + " value='" + data[i].params[k].key + "'/>"
				                      ...
				                      + " value='" + data[i].params[k].name + "'/>"
												...
												+ "' name='value' type='text' value=" + data[i].params[k].value + "></div></div>";
									}
								}
...
<script type="text/javascript">
	...
	function ajaxSubmitForm() {
		...
		$
				.ajax({
					url : "${pageContext.request.contextPath }/addPayclassMerchantRule",
					type : "post",
